...
public abstract class QuestionStump(Enum enumValue, SouvenirModule souvenir, string[] args)
{
    ...
    public QandAStump Answers(Sprite correct, Sprite[] preferredWrong = null, Sprite[] all = null, float xStretch = 1f) => Answers([correct], preferredWrong, all, xStretch);
    public QandAStump Answers(Sprite[] correct, Sprite[] preferredWrong = null, Sprite[] all = null, float xStretch = 1f) => new(this, new SpriteAnswerStump(correct, preferredWrong, all ?? EnumValue.GetAllSprites(Souvenir), xStretch));
    ...
    public abstract QuestionBase MakeQuestion(string moduleFormat, QuestionExtra questionExtraFromDiscriminator);
