...
public partial class SouvenirModule
{
    ...
    private IEnumerable<object> ProcessLongWords(KMBombModule module)
    {
        ...
        var fldSolved = GetField<bool>(comp, "ModuleSolved");
        var possibleWords = GetField<List<string>>(comp, "SixLetterWords").Get().ToArray();
        var word = GetField<string>(comp, "chosenSixLetterWord").Get();

        while(!fldSolved.Get())
            yield return new WaitForSeconds(.1f);
        _modulesSolved.IncSafe(_LongWords);

        addQuestion(module, Question.LongWordsWord, correctAnswers: new[] { word }, preferredWrongAnswers: possibleWords);
    }
}