...
public partial class SouvenirModule
{
    ...
    private IEnumerator<SouvenirInstruction> ProcessHolographicMemory(ModuleData module)
    {
        ...
        var answerSprites = GetField<int[][]>(comp, "ans").Get().Select(ans => HolographicMemorySprites[symbolSelected[16 * ans[2] + 4 * ans[0] + ans[1]]]).ToArray();

        for (var side = 0; side < 2; side++)
            for (var row = 0; row < 4; row++)
                for (var col = 0; col < 4; col++)
                    yield return question(SHolographicMemory.InitialGrid, questionSprite: HolographicMemorySprites[symbolSelected[16 * side + 4 * row + col]]).Answers(side == 0 ? "Light" : "Dark");
    }
}
