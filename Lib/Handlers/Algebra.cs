...
public enum SAlgebra
{
    ...
    [SouvenirDiscriminator("the Algebra where the first equation was {0}", Arguments = ["a=3z", "a=5+y", "a=6-x", "a=7x", "a=8y", "a=9+z", "a=x/2", "a=x+1", "a=y/4", "a=y-2", "a=z/10", "a=z-7"], ArgumentGroupSize = 1)]
    Discriminator1,

    [SouvenirDiscriminator("the Algebra where the second equation was {0}", Arguments = ["b=(2x/10)-y", "b=(7x)y", "b=(x+y)-(z/2)", "b=(y/2)-z", "b=(zy)-(2x)", "b=(z-y)/2", "b=2(z+7)", "b=2z+7", "b=xy-(2+x)", "b=xyz"], ArgumentGroupSize = 1)]
    Discriminator2
}
