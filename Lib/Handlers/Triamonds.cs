using System.Collections.Generic;
using Souvenir;

using static Souvenir.AnswerLayout;

public enum STriamonds
{
    [Question("What colour triangle pulsed {1} in {0}?", ThreeColumns6Answers, "black", "red", "green", "yellow", "blue", "magenta", "cyan", "white", Arguments = [QandA.Ordinal], ArgumentGroupSize = 1, TranslateAnswers = true)]
    PulsingColours
}

public partial class SouvenirModule
{
    [Handler("triamonds", "Triamonds", typeof(STriamonds), "Kuro")]
    [ManualQuestion("What was the pulsing colour sequence?")]
    private IEnumerator<SouvenirInstruction> ProcessTriamonds(ModuleData module) => processPolyiamonds(module, "triamondsScript", STriamonds.PulsingColours, new[] { "black", "red", "green", "yellow", "blue", "magenta", "cyan", "white" });
}