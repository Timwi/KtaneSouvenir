...
public enum SSonicTheHedgehog
{
    ...
    [SouvenirQuestion("Which sound was played by the {1} screen on {0}?", TwoColumns4Answers, Type = AnswerType.Audio, ForeignAudioID = "sonic", AudioSizeMultiplier = 4, Arguments = ["Running Boots", "Invincibility", "Extra Life", "Rings"], TranslateArguments = [true], ArgumentGroupSize = 1)]
    Sounds
}
...
public partial class SouvenirModule
{
    ...
    [SouvenirManualQuestion("Which sound was played by each screen?")]
    [SouvenirManualQuestion("What were the pictures?")]
    private IEnumerator<SouvenirInstruction> ProcessSonicTheHedgehog(ModuleData module)
    {
        ...
        var soundNameMapping = "boss|breathe|bumper|continueSFX|drown|emerald|extraLife|finalZone|invincibleSFX|jump|lamppost|marbleZone|skid|spikes|spin|spring"
            .Split('|').ToDictionary(s => s, s => Sounds.GetForeignClip("sonic", s));
        ...
        var preferredWrong = sounds.Select(s => soundNameMapping[s]).ToArray();
        var all = soundNameMapping.Values.ToArray();
        for (var screen = 0; screen < 4; screen++)
            yield return question(SSonicTheHedgehog.Sounds, args: [screenNames[screen]])
                .Answers(soundNameMapping[sounds[screen]], preferredWrong: preferredWrong, all: all);
    }
}
