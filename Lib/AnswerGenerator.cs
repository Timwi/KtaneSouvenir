...
///         the documentation for these classes for more details.</para></remarks>
/// <example>
///     <code>
...
public static class AnswerGenerator
{
    ...
    /// <remarks>
    ///     <para>
    ///         This generator generates string representations of numbers within a range specified by inclusive bounds and a
    ///         step size. The numbers may optionally be formatted using a format string. For more details, see
    ///         https://docs.microsoft.com/en-us/dotnet/api/system.int32.tostring?view=netframework-3.5.</para>
    ///     <para>
    ///         If a step size is specified, the generated numbers will be multiples of the step size plus the minimum. The
    ///         maximum may not be included if the step size is greater than 1.</para></remarks>
    /// <example>
    ///     <code>
    ...
    /// <remarks>
    ///     <para>
    ///         This generator builds answers using a specified number of random characters from one or more lists. Each list
    ///         specifies a combination of individual characters and ranges, and chooses a specified number of characters from
    ///         that list, with replacement.</para></remarks>
    /// <example>
    ///     <code>
    ...
    public class Strings : AnswerGeneratorAttribute<string>
    {
        public struct CharacterList
        {
            ...
            public static CharacterList Parse(string expression)
            {
                ...
                if (i > 0 && int.TryParse(expression.Substring(0, i), NumberStyles.AllowLeadingWhite | NumberStyles.AllowTrailingWhite, null, out var count))
                {
            ...
            public readonly int OptionCount
            {
                get
                {
                    ...
                    if (Ranges != null)
                    {
                        for (var i = 0; i < Ranges.Length; i += 2) n += Ranges[i + 1] + 1 - Ranges[i];
                    }
                    return n;
                }
            }
        ...
        private readonly CharacterList[] _characterLists;

        /// <param name="characterLists">
        ///     A list of expressions specifying character lists.</param>
        /// <remarks>
        ///     <para>
        ///         A character list expression specifies a list of characters and a number of characters to choose from that
        ///         list. A character followed by a hyphen ('-') and another character specifies a range, including all
        ///         characters between the bounds inclusively. Other characters are directly included in the list. A hyphen
        ///         may be included in the list by placing it at the start or end of the string, or immediately after a range.</para>
        ///     <para>
        ...
        {
            _characterLists = new CharacterList[characterLists.Length];
            for (var i = characterLists.Length - 1; i >= 0; i--)
                _characterLists[i] = CharacterList.Parse(characterLists[i]);
        }
        /// <param name="count">
        ...
        public static Strings FromCharacterLists(params CharacterList[] characterLists) => new(characterLists);
        ...
        public override IEnumerable<string> GetAnswers(SouvenirModule module)
        {
            ...
            while (true)
            {
                foreach (var characterList in _characterLists)
    ...
    public sealed class Wumbo : AnswerGeneratorAttribute<string>
    {
        public override IEnumerable<string> GetAnswers(SouvenirModule module)
        {
            var primes = new[] { 2uL, 3uL, 5uL, 7uL, 11uL, 13uL };
            while (true)
            {
                ulong n = 1;
    ...
    public sealed class AMessage : AnswerGeneratorAttribute<string>
    {
        ...
        public override IEnumerable<string> GetAnswers(SouvenirModule module)
        {
            while (true)
            {
                var m = Random.Range(0, _message.Length - 5);
                yield return Random.Range(0, 2) == 0 ? _message.Substring(m, 5) : new string(_message.Substring(m, 5).Reverse().ToArray());
            }
        }
        public override int Count => 2 * (_message.Length - 4);
    }
    ...
    public class Polyominoes(int minCellCount = 1, int maxCellCount = -1, int minWidth = 1, int maxWidth = -1, int minHeight = 1, int maxHeight = -1, bool requireSingleStrokePath = false, bool excludeEqualRotations = false) : AnswerGeneratorAttribute<Sprite>
    {
        private readonly int _minCellCount = minCellCount;
        private readonly int _maxCellCount = maxCellCount == -1 ? minCellCount : maxCellCount;
        private readonly int _minWidth = minWidth;
        private readonly int? _maxWidth = maxWidth == -1 ? null : maxWidth;
        private readonly int _minHeight = minHeight;
        private readonly int? _maxHeight = maxHeight == -1 ? null : maxHeight;
        private readonly bool _requireSingleStrokePath = requireSingleStrokePath;
        ...
        public override IEnumerable<Sprite> GetAnswers(SouvenirModule module)
        {
            foreach (var poly in Cached.Shuffle())
                yield return Sprites.GeneratePolyominoSprite(poly.Width, poly.Indices);
        }
        ...
        private static PolyominoData NormalizeCells(IEnumerable<(int x, int y)> cells)
        {
            ...
            var minX = arr.Min(c => c.x);
            var minY = arr.Min(c => c.y);

            var shifted = arr.Select(c => (x: c.x - minX, y: c.y - minY)).ToArray();

            var width = shifted.Max(c => c.x) + 1;
            var height = shifted.Max(c => c.y) + 1;
            var indices = shifted.Select(c => c.x + width * c.y).OrderBy(i => i).ToArray();
        ...
        private static bool HasSingleStrokePath(PolyominoData poly)
        {
            ...
            var neighbors = cells.ToDictionary(c => c, c => new[] { (c.x - 1, c.y), (c.x + 1, c.y), (c.x, c.y - 1), (c.x, c.y + 1) }.Where(cellSet.Contains).ToArray());
