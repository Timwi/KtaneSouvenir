...
public class CoordAnswerStump(Coord[] correct, Coord[] preferredWrong, Coord[] all) : AnswerStump<Coord>(correct, preferredWrong, all)
{
    ...
    protected override AnswerSet MakeAnswerSet(Coord[] answers, int correctIndex, QuestionAttribute qAttr, SouvenirModule souvenir) => new CoordAnswerSet(qAttr.Layout, answers, correctIndex);
